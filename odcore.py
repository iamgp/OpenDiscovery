...
# @name:  try_remove
# @args:  filename
# @usage: tries to delete a file/folder
...
# @name:    can_write
# @args:    path
# @usage:   checks if append tag is active and if a folder exists
def can_write(path):
...
def use_obabel(inputType, outputType, obabel_extra=''):
    ...
    for cmpnd in glob.glob('{0}/*.{1}'.format(inputType, inputType)):
        ...
        subprocess.call('obabel {0} -O {1}/{2}.{3} {4}'.format(cmpnd, outputType, b, outputType, obabel_extra), stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
...
def use_obminimize(inputFolder):
    ...
    for cmpnd in glob.glob('{0}-minimised/*'.format(inputFolder)):
        ...
        subprocess.call('obminimize -sd -c 1e-5 {0}'.format(cmpnd), stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
